...
from HomePageUser.models import PersonalTask
...
def shared_calendar(request):
    ...
    first_weekday = first_day_of_month.weekday()  # 0=Пн
    # --- Собираем задачи ---
    family_tasks = FamilyTask.objects.filter(family=family, date__in=week_dates)
    personal_tasks = PersonalTask.objects.filter(user=request.user, date__in=week_dates)
    # Объединяем задачи по дате
    tasks_by_date = defaultdict(list)
    for task in family_tasks:
        tasks_by_date[task.date].append({'type': 'family', 'task': task})
    for task in personal_tasks:
        tasks_by_date[task.date].append({'type': 'personal', 'task': task})
    # Для мини-календаря
    family_month_tasks = FamilyTask.objects.filter(family=family, date__gte=first_day_of_month, date__lte=last_day_of_month)
    personal_month_tasks = PersonalTask.objects.filter(user=request.user, date__gte=first_day_of_month, date__lte=last_day_of_month)
    tasks_by_month_date = defaultdict(list)
    for task in family_month_tasks:
        tasks_by_month_date[task.date].append({'type': 'family', 'task': task})
    for task in personal_month_tasks:
        tasks_by_month_date[task.date].append({'type': 'personal', 'task': task})
    # Для стрелок
    if month == 1:
        prev_month = 12
        prev_year = year - 1
